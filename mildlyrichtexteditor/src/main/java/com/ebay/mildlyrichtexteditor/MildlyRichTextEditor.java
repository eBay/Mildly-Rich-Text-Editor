...
public class MildlyRichTextEditor extends AppCompatEditText implements CompoundButton.OnCheckedChangeListener {
     ...
     */
    public enum Style {
        BOLD("Bold"), ITALIC("Italic"), UNDERLINE("Underline"), FONT_SIZE("Font Size"), SERIF("Serif"), SANS_SERIF("Sans serif"), MONOSPACE("Monospace");
    ...
    public void setFontSizeButton(ImageButton button, final View menu, List<ToggleButton> buttons) {
        ...
        for (ToggleButton t : fontSizeButtons) {
            if (DEFAULT_ABSOLUTE_SIZE.equals(t.getText().toString())) t.setChecked(true); //default
        ...
        if (!TextUtils.isEmpty(html)) {
            ...
            for (CharacterStyle style : styles) {
                if (style instanceof UnderlineSpan) {
                    ...
                    editableDescription.setSpan(new CustomUnderlineSpan(), spanStart, spanEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
            }
    ...
    protected void onSelectionChanged(int selStart, int selEnd) {
        ...
        if (boldButton == null || italicsButton == null || underlineButton == null || fontSizeButtons == null) return;
        ...
        if (selStart > 0 && selStart == selEnd) { //user moved cursor
            ...
            for (CharacterStyle styleSpan : styleSpans) {
                if (styleSpan instanceof StyleSpan) {
                    if (((StyleSpan) styleSpan).getStyle() == Typeface.BOLD) boldExists = true;
                    else if (((StyleSpan) styleSpan).getStyle() == Typeface.ITALIC) italicsExists = true;
                    else if (((StyleSpan) styleSpan).getStyle() == Typeface.BOLD_ITALIC)
                        boldExists = italicsExists = true;
                } else if (styleSpan instanceof CustomUnderlineSpan) underlinedExists = true;
    ...
    class RichTextEditorTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable e) {
            if (TextUtils.isEmpty(e.toString().trim())) {
                ...
                for (ToggleButton t : fontSizeButtons) {
                    t.setOnCheckedChangeListener(null);
                    if (DEFAULT_ABSOLUTE_SIZE.equals(t.getText().toString())) t.setChecked(true);
            ...
            CharacterStyle[] appliedStyles = e.getSpans(position - getPositionOffsetSafe(position), position, CharacterStyle.class);
            ...
            for (CharacterStyle appliedStyle : appliedStyles) //identify styles applied to entered text
            {
                ...
                else if (appliedStyle instanceof RelativeSizeSpan) appliedSizeSpan = (RelativeSizeSpan) appliedStyle;
            }
            ...
            if (boldButton != null && boldButton.isChecked()) {
                ...
                if (currentBoldSpan == null)
                    ...
                    e.setSpan(new StyleSpan(Typeface.BOLD), position - getPositionOffsetSafe(position), position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                else {
                ...
                if (boldStart <= (position - getPositionOffsetSafe(position)))
                    e.setSpan(new StyleSpan(Typeface.BOLD), boldStart, position - getPositionOffsetSafe(position), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            ...
            if (italicsButton != null && italicsButton.isChecked()) {
                if (currentItalicSpan == null)
                    e.setSpan(new StyleSpan(Typeface.ITALIC), position - getPositionOffsetSafe(position), position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                else {
                ...
                if (italicStart <= (position - getPositionOffsetSafe(position))) {
                    e.setSpan(new StyleSpan(Typeface.ITALIC), italicStart, position - getPositionOffsetSafe(position), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                }
            ...
            if (underlineButton != null && underlineButton.isChecked()) {
                if (currentUnderlineSpan == null)
                    e.setSpan(new CustomUnderlineSpan(), position - getPositionOffsetSafe(position), position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                else {
                ...
                if (underLineStart <= (position - positionOffset))
                    e.setSpan(new CustomUnderlineSpan(), underLineStart, position - getPositionOffsetSafe(position), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            ...
            for (ToggleButton t : fontSizeButtons) {
                if (t.isChecked()) selectedRelativeSize = Float.parseFloat(t.getText().toString()) / 14f;
            }
            ...
            if (selectedRelativeSize != null && Float.compare(currentRelativeSize, selectedRelativeSize) != 0) {
                if (appliedSizeSpan == null)
                    ...
                    e.setSpan(new RelativeSizeSpan(selectedRelativeSize), position - getPositionOffsetSafe(position), position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                else {
                    e.removeSpan(currentBoldSpan);
                    e.setSpan(new RelativeSizeSpan(selectedRelativeSize), position - getPositionOffsetSafe(position), position, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                }
            } else if (selectedRelativeSize != null && Float.compare(currentRelativeSize, selectedRelativeSize) != 0 && appliedSizeSpan != null) {
                ...
                if (sizeStart <= (position - positionOffset))
                    e.setSpan(new RelativeSizeSpan(currentRelativeSize), sizeStart, position - getPositionOffsetSafe(position), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);

                e.setSpan(new RelativeSizeSpan(selectedRelativeSize), position - getPositionOffsetSafe(position), sizeEnd, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                ...
                if (this.prevStyles.length > 0) {
                    ...
                    for (CharacterStyle appliedStyle : this.prevStyles) {
                        if (appliedStyle instanceof StyleSpan) {
                            ...
                            String absoluteSize = Integer.toString(Math.round(appliedSizeSpan.
                                    getSizeChange() * 14f));
                            for (ToggleButton t : fontSizeButtons) {
                                if (absoluteSize.equals(t.getText().toString())) t.setChecked(true);
                            }
                        }
                    }
                } else {
